...
from zorna.site.models import SiteRegistration, REGISTRATION_NO_VALIDATION,REGISTRATION_EMAIL_VALIDATION, REGISTRATION_ADMIN_VALIDATION
...
class AccountEditProfileForm(forms.Form):
    ...
    email = forms.EmailField(widget=forms.TextInput(attrs={'size':80}),label=_(u'Email address'))
    firstname = forms.CharField(widget=forms.TextInput(attrs={'size':80}), label=_(u'Firstname'))
    lastname = forms.CharField(widget=forms.TextInput(attrs={'size':80}),label=_(u'Lastname'))
    is_active = forms.BooleanField(label=_('is active'), required=False, help_text=_("Designates whether this user should be treated as active. Unselect this instead of deleting accounts."))
    ...
    def save(self, request, admin_origin, is_active, *args, **kwargs):
        ...
        if admin_origin == False and reg:
            ...
            if reg.validation_type == REGISTRATION_EMAIL_VALIDATION:
                ...
                body_text = render_to_string('account/activation_email.html', ec )
                ec['mime_type'] = 'html'
                body_html = render_to_string('account/activation_email.html', ec )
                email = ZornaEmail()
                email.append(subject, body_text, body_html, settings.DEFAULT_FROM_EMAIL, [new_user.email])
                email.send()
            elif reg.validation_type == REGISTRATION_ADMIN_VALIDATION:
                from zorna.site.models import SiteOptions
                email_lists = SiteOptions.objects.get_authorized_user('zorna_validate_registration')
                if email_lists:
                    subject = _("Your site has recorded a new registration")
                    body_text = render_to_string('account/registration_email.html', ec )
                    ec['mime_type'] = 'html'
                    body_html = render_to_string('account/registration_email.html', ec )
                    email = ZornaEmail()
                    email.append(subject, body_text, body_html, settings.DEFAULT_FROM_EMAIL, [u.email for u in email_lists])
                    email.send()
        return new_user
   
class UserGroupForm(ModelForm):
    parent = TreeNodeChoiceField(queryset=UserGroup.objects.filter(id__gte=3).order_by('lft'), required = True) #@UndefinedVariable
...
class UserAvatarForm(forms.ModelForm):
    ...
    def clean_avatar(self):
        ...
        if data:
            ...
            raise forms.ValidationError(_(u"Please, choose a valid image file"))
        return data
...
class UsersFormImportCsv(forms.Form):
    csv_file = forms.CharField(label=_(u'File'), widget=forms.FileInput(attrs={'size':'80'}))
...
class RequestNewPasswordForm(forms.Form):
    username = forms.CharField(widget=forms.TextInput(attrs={'size':'80'}), label=_(u'Username'), help_text=_(u'Username or email address'))
