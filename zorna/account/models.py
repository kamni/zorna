...
class UserProfile(models.Model):
    ...
    user = models.ForeignKey(User, null=True, blank=True, related_name='user_profile')
    activation_key = models.CharField(_('activation key'), max_length=40, editable=False)
    reset_password_key = models.CharField(_('activation key'), max_length=40, editable=False, default='')
    #To minimize database writes, the last_activity field is only updated once per day
...
class UserAvatar(models.Model):
    ...
    avatar = models.ImageField(storage=fs, max_length=1024, upload_to=get_avatar_filepath, blank=True)
