...
def forgot_passord(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            user = User.objects.get(Q(username=form.cleaned_data['username'])|Q(email=form.cleaned_data['username']))
            up = user.get_profile()
            ...
            body_text = render_to_string('account/email_reset_password_text.html', ec )
            body_html = render_to_string('account/email_reset_password_html.html', ec )
            email.append(subject, body_text, body_html, settings.DEFAULT_FROM_EMAIL, [user.email])
            email.send()
...
def register(request, admin_origin = False, is_active = False, success_url=None, template="account/registration_form.html"):
    ...
    if not admin_origin:
        try:
            ...
            if reg.validation_type == REGISTRATION_ADMIN_VALIDATION:
                register_complete_url = reverse('user_register_complete_noemail')
            elif reg.validation_type == REGISTRATION_NO_VALIDATION:
                register_complete_url = reverse('user_register_complete_novalidation')
    ...
    try:
        ...
        if f.bind_to_account:
            ...
            form_for_form.fields['zorna_owner-id']= forms.CharField(widget=forms.HiddenInput(), required=False)
        lform = form_for_form
    ...
    if request.user.is_anonymous():
        lform.fields['REMOTE_ADDR']= forms.CharField(widget=forms.HiddenInput(), initial=request.META['REMOTE_ADDR'])
        lform.fields['REMOTE_ADDR']= forms.CharField(widget=forms.HiddenInput(), initial=request.META['REMOTE_ADDR'])
        lform.fields['captcha']= ReCaptchaField(label='Captcha', required=True, widget=ReCaptchaWidget())
...
def edit_user(request, user):
    baccess = SiteOptions.objects.is_access_valid(request.user, 'zorna_validate_registration')
        ...
        try:
            ...
            if form.bind_to_account:
                if not form_for_form.instance.id: 
                    del form_for_form.fields['zorna_owner']
                    del form_for_form.fields['zorna_owner-id']
                    form_for_form.fields['zorna_owner-id']= forms.CharField(widget=forms.HiddenInput(), initial=user)
                else:
                    del form_for_form.fields['zorna_owner']
        except:
            form_for_form = None
        ...
        if request.method == 'POST':
            if form.is_valid():
                if not form_for_form or form_for_form.is_valid():
                    ...
                    user.save()
                    try:
                        cal = Calendar.objects.get_calendar_for_object(user, 'owner')
...
def list_users(request):
    ...
    if request.user.is_superuser or baccess:
        ...
        if q != '':
            user_lists = User.objects.filter(Q(first_name__icontains=q) | Q(last_name__icontains=q)).order_by('first_name', 'last_name')
        else:
...
def list_groups(request):
    if request.user.is_superuser:
        group_lists = UserGroup.objects.filter(id__gt=defines.ZORNA_GROUP_REGISTERED).order_by('lft')
...
def users_group(request, group):
    if request.user.is_superuser:
        ...
        if request.method == 'POST' and grp:
            selected = request.POST.getlist('_selected_action')
            args = [obj for obj in UserProfile.objects.filter(user__in=selected)]
        ...
        group_lists = User.objects.filter(pk__in=extra_context['users_groups']).order_by('first_name')
...
def user_settings(request):
    ...
    if entry:
        ...
        c = RequestContext(request, {'row': row, 'columns':columns, 'account':request.user})
    ...
    try:
        ...
        if form.bind_to_account:
            if not form_for_form.instance.id: 
                del form_for_form.fields['zorna_owner']
                del form_for_form.fields['zorna_owner-id']
                form_for_form.fields['zorna_owner-id']= forms.CharField(widget=forms.HiddenInput(), initial=request.user.pk)
            else:
                del form_for_form.fields['zorna_owner']
    except:
        form_for_form = None
...
def user_settings_avatar(request):
    if request.method == 'POST':
        ...
        if request.POST.has_key('bavatar'):
        ...
        elif request.POST.has_key('bdelavatar'):
            try:
...
def list_administrators(request):
    if request.user.is_superuser:
        ...
        admin_lists = User.objects.filter(is_superuser=1).order_by('first_name')
    ...
    if tempfile:
        ...
        for row in csv_reader:
            if nb_row == 0:
                ...
                username = smart_unicode(row[fields['username']].strip(), encoding)
                first_name = smart_unicode(row[fields['first_name']].strip(), encoding)
                last_name = smart_unicode(row[fields['last_name']].strip(), encoding)
                ...
                if username and first_name and last_name and email and password:
                    if registered_users.has_key(username): #user already exist, update record
        ...
        c = RequestContext(request, {'users_error': users_error, 'users_added':users_added, 'users_updated':users_updated})
...
def form_map_import_fields(request, tempfile, separator, encoding='utf-8', message='' ):
    if request.user.is_superuser:
        try:
            form_profile = FormsForm.objects.get(slug=settings.ZORNA_USER_PROFILE_FORM)
        except:
            form_profile = None
        ...
        extra_context['user_field_choices'] = [
                                               {'id': 'username', 'label': _("Nickname")},
                                               {'id': 'first_name', 'label': _("First name")},
                                               {'id': 'last_name', 'label': _("Last name")},
                                               {'id': 'email', 'label': _("E-mail address")},
                                               {'id': 'password', 'label': _("Password")},
                                               ]
...
def import_users_view(request):
    if request.user.is_superuser:
        form_csv = UsersFormImportCsv(request.POST or None, request.FILES or None)
        if request.method == 'POST':
            ...
            elif form_csv.is_valid() and request.FILES.has_key('csv_file'):
                import tempfile
