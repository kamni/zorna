...
REGISTRATION_VALIDATION_TYPES = (
    (REGISTRATION_EMAIL_VALIDATION, _('Confirm account by validating address email')),
    (REGISTRATION_ADMIN_VALIDATION, _('Manual confirmation by administrators')),
    (REGISTRATION_NO_VALIDATION, _('Confirm account without address email validation')),
)

class SiteRegistration(ZornaEntity):
    allow_registration = models.BooleanField(_('allow registration'), default=False, editable=True,
                                  help_text=_('Enable this option to let users register on your site'))
    validation_type = models.IntegerField(_('validation type'), max_length=1, choices=REGISTRATION_VALIDATION_TYPES, default=0)
    groups = models.ManyToManyField(UserGroup, verbose_name=_('groups'), blank=True, editable=False)
...
def handler_views_was_called(**kwargs):
    ...
    for k, o in SITES_OPTIONS.iteritems():
        if options.has_key(k):
            ...
            opt.save()
    if len(options):
        SiteOptions.objects.filter(pk__in=[opt.pk for opt in options.values() ]).delete()
