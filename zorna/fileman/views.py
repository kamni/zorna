...
from zorna.acl.models import get_allowed_objects, get_acl_for_model, get_acl_by_object
...
def notify_users(request, folder, files, upload=True):
    if folder.inherit_permissions:
        ...
        for f in parents:
            if f.inherit_permissions == False:
                folder = f
                break
    users_list = get_acl_by_object(folder, 'reader')
    email = ZornaEmail()
    url = request.build_absolute_uri(reverse('documents', args=[])) + '?dir=F%s' % folder.pk
    ec = {"folder": folder, "files": files, 'url': url, 'upload': upload, 'user': request.user}
...
def documents(request):
    if request.user.is_authenticated:
        ...
        if request.user.is_anonymous():
            ...
            extra_context['bpersonal_folder'] = SiteOptions.objects.is_access_valid(request.user, 'zorna_personal_documents')
        ...
        ao = get_allowed_shared_folders(request.user, ['reader', 'manager', 'writer'])
        if ao:
...
def get_sub_folders(dir_path, user=None):
    ...
    if dirs[0][0] == 'F' and len(dirs) == 1:
        allowed_objects = get_allowed_shared_folders(user, ['reader', 'manager', 'writer'])
        ob_list = ZornaFolder.objects.filter(Q(pk__in=allowed_objects) | Q(inherit_permissions=True), parent__pk=dirs[0][1:])
        for f in ob_list:
...
def dirlist_personal_folders(request):
    ...
    for d in ret:
        r.append('<li class="directory collapsed"><a href="#" rel="%s/" title="%s/">%s</a></li>' % (d['rel'], d['rel'], d['name']))
...
def dirlist_communities_folders(request):
    ...
    if path:
        ...
        root_path = get_upload_library()
        ret = []
        com = Community.objects.filter(pk__in=allowed_objects, enable_documents=True)
        for c in com:
            path = u"C%s" % c.pk
...
def dirlist_shared_folders(request):
    ...
    if path:
        ...
        allowed_objects = get_allowed_shared_folders(request.user, ['reader', 'manager', 'writer'])
        ret = []
        ob_list = ZornaFolder.objects.filter(pk__in=allowed_objects, parent__isnull=True)
        root_path = get_upload_library()
        for f in ob_list:
            path = u"F%s" % f.pk
            fullpath = u"%s/%s" % (root_path, path)
...
def get_files(request, path, search_tag, search_text):
    ...
    if search_text:
        if filterreq:
            ...
            filterreq = Q(description__icontains=search_text) | Q(tags__icontains=search_text)
            ...
            if what == 'rename' and len(parts) > 1:
                ...
                if new_folder == n_folder:
                    try:
                        ...
                        path_src = u"%s/%s" % (root_path, urllib.unquote(path_dest))
                        dest = parts[:-1]
                        dest = '/'.join(dest)
                        path_dest = u"%s/%s/%s" % (root_path, urllib.unquote(dest), n_folder)
                        os.rename(path_src, path_dest)
                        ret['status'] = 'success'
                        ret['message'] = gettext(u'Folder renamed successfully')
                        ret['data'] = u"%s/%s/" % (urllib.unquote(dest), n_folder)
                    except Exception as e:
                        ret['status'] = 'error'
                        ret['message'] = e
                else:
                ...
                if new_folder == n_folder:
                    try:
                        ...
                        os.mkdir(path)
                        ret['status'] = 'success'
                        ret['message'] = gettext(u'Folder created successfully')
                        ret['data'] = '<li class="directory collapsed"><a href="#" rel="%s/">%s</a></li>' % (rel, n_folder)
                    except Exception as e:
                        ret['status'] = 'error'
                        ret['message'] = e
                else:
        ...
        if path != pathfile:
            ...
            if bmanager == False:
                ...
                try:
                    ...
                    fname = slugify(s[0])
                    fext = s[1]
                    dest = u"%s/%s/%s" % (get_upload_library(), path, u"%s,%s%s" % (tab[0], fname, fext))
                    ...
                    if form_set.is_valid():
                        ...
                        for i in range(0, form_set.total_form_count()):
                            ...
                            try:
                                ...
                                upload_files[fname] = {'pk':'', 'file':f, 'description':form.cleaned_data['description'], 'tags':form.cleaned_data['tags'] }
                            except:
                                continue
                        path = u"%s/%s" % (root_path, urllib.unquote(path_dest))
                        bupload = False
                        for f, info in upload_files.iteritems():
                            file = ZornaFile(owner=request.user, modifier=request.user, description=info['description'], tags=info['tags'], folder=path_dest.split('/')[0])
                            file.save()
                            destination = open(u"%s/%s,%s" % (path, file.pk, f), 'wb+')
                            bupload = True
                            for chunk in info['file'].chunks():
                                destination.write(chunk)
                            destination.close()
                            ifiles.append({'name': f, 'description': info['description'] })
                        if bupload:
                            ret['message'] = gettext(u"Files uploaded successfully")
                            # # notify users
                        ...
                        c = RequestContext(request, {'form_set':form_set, 'cdir_components': cdir_components, 'folder_dest': pathfile, 'folder_content_url': get_url_folder_content(pathfile)})
                        ret['data'] = t.render(c)
                        json_data = simplejson.dumps(ret)
                        return HttpResponse('<textarea>' + json_data + '</textarea>')
                else:
                ...
                if request.method == 'POST':
                    ...
                    if form.is_valid():
                        ...
                        if int(chunk) + 1 >= int(chunks):
                            ...
                            fname = u"%s%s" % (slugify(s[0]), s[1])
                            file = ZornaFile(owner=request.user, modifier=request.user, description='', folder=path_dest.split('/')[0])
                            file.save()
                    ...
                    ret['status'] = 'success'
                t = loader.get_template('fileman/upload_multiple_documents.html')
        ...
        if path_dest != ppath:
            ...
            if bmanager == False:
                ...
                try:
                    ...
                    if request.method == 'POST':
                        ...
                        if form.is_valid():
                            ...
                            if request.FILES:
                                ...
                                s = os.path.splitext(f.name)
                                fname = u"%s%s" % (slugify(s[0]), s[1])
                                os.remove(u"%s/%s/%s,%s" % (root_path, path_dest, file_id, file_name))
                                destination = open(u"%s/%s/%s,%s" % (root_path, path_dest, file_id, fname), 'wb+')
                                ...
                                bnotify = request.POST.get('notify_users', 0)
                                if folder and folder.email_notification and (folder.email_notification == 1 or bnotify):
                                    notify_users(request, folder, [{'name': fname, 'description': fo.description }], False)

                            ret['message'] = gettext(u"File updated successfully")
                            ret['status'] = 'success'
                            json_data = simplejson.dumps(ret)
                            return HttpResponse('<textarea>' + json_data + '</textarea>')
                        else:
                    ...
                    ec = { 'file':fo, 'form':form, 'file_path':pathfile, 'file_name': file_name,
...
def add_files(root_folder, user, path):
    ...
    for f in os.listdir(path):
        ...
        if pk == False:
            try:
                file = ZornaFile(description='', owner=user, modifier=user, folder=root_folder)
                file.save()
