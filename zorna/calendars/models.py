...
class EventDetails(models.Model):
    ...
    bgcolor = models.CharField(_('background color'), max_length=6, default='', blank=True,help_text = _("If used, calendar category is ignored."))
    location = models.CharField(_('location'), max_length=100, blank=True)
    free_busy = models.BooleanField(_('Busy'), editable=True)
    privacy = models.IntegerField(_('privacy'), max_length=1, choices=PRIVACY_TYPES, default=0)
...
class ZornaCalendarManager(models.Manager):
    ...
    def get_or_create_calendar_for_object(self, user, content_object, distinction = None, name = None):
        calendar = Calendar.objects.get_or_create_calendar_for_object(content_object, distinction, name)
        try:
            return ZornaCalendar.objects.get(object_id=content_object.pk, calendar=calendar)
        except ZornaCalendar.DoesNotExist:
            zcalendar = ZornaCalendar(content_object=content_object, owner=user)
...
class ZornaCalendar(ZornaEntity):
    ...
    secret_key = models.CharField(_('secret key'), max_length=40, editable=False, default='')
    ct_type = models.ForeignKey(ContentType, editable=False)
    object_id = models.IntegerField(editable=False)
    content_object = generic.GenericForeignKey('ct_type', 'object_id')
    custom_view = models.CharField(_("custom view"), max_length = 10, choices=CALENDAR_CUSTOM_VIEW_CHOICES, default='month')
    week_start_on = models.IntegerField(_("week start on"), max_length = 1, choices=CALENDAR_WEEK_START_ON_CHOICES, default='0')
    default_meeting_length = models.IntegerField(_("default meeting length"), max_length = 2, 
                            choices=[(i, i) for i in [10, 15, 20, 30, 45, 60, 120]], 
                            default='60', help_text= _("Default meeting length in minutes"))
    time_scale = models.IntegerField(_("default time scale"), max_length = 2, 
                            choices=[(i, i) for i in [5, 10, 15, 30, 60]], 
