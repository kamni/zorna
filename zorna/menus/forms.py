...
class ZornaMenuItemBaseForm(ModelForm):
    parent = TreeNodeChoiceField(queryset=ZornaMenuItem.tree.filter(), required = False, empty_label="-- Nothing --")
    ...
    def clean_slug(self):
        if 'slug' in self.cleaned_data and 'name' in self.cleaned_data:
            ...
            raise forms.ValidationError(_(u'You must provide a slug or a menu item name'))

class ZornaMenuItemContainerForm(ZornaMenuItemBaseForm):
    pass

class ZornaMenuItemUrlForm(ZornaMenuItemBaseForm):
    url = forms.CharField(widget=forms.TextInput(attrs={'size':80}), label=_(u'Url'), required=False) 
        
class ZornaMenuItemArticleCategoryForm(ZornaMenuItemBaseForm):
    category = TreeNodeChoiceField(queryset=ArticleCategory.tree.filter(), required = True)
...
class ZornaMenuItemPageContentForm(ZornaMenuItemBaseForm):
    ...
    def pages_as_choices(self, path=''):
        root_path = settings.PROJECT_PATH +os.sep+ settings.ZORNA_CONTENT + os.sep
        ...
        for root, dirs, files in os.walk(root_path):
            for f in files:
                ...
                pages.append(('/content/' + fpath[len_root_path:], fpath[len_root_path:])) 
        if not path:
...
class ZornaMenuItemFormsForm(ZornaMenuItemBaseForm):
    form = forms.ModelChoiceField(queryset=FormsForm.objects.all(), required=True)
