...
class FaqQuestionQuerySet(QuerySet):
    """
    ...
    def active(self, **kwargs):
        ...
        if kwargs.get('slug'):
            slug = kwargs['slug']
            return self.filter(status__exact=defines.STATUS_ACTIVE, slug__exact=slug)
        elif group:
            return self.exclude(status__exact=defines.STATUS_INACTIVE,)
        else:
            return self.filter(status__exact=defines.STATUS_ACTIVE,)
...
class FaqQuestionManager(models.Manager):
    ...
    def get_query_set(self):
        return FaqQuestionQuerySet(self.model)
