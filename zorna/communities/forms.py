...
class MessageCommunityForm(forms.Form):
    ...
    send_to = forms.CharField(widget=forms.TextInput(attrs={'class':'com_recipients'}), label=_(u'Recipients'))
    attachments = forms.CharField(label=_(u'Attachments'), widget=forms.FileInput(attrs={'size':'20', 'class':'multi', 'maxlength':'2'}), required=False)
        ...
        if send_to:
            ...
            for item in send_to:
                ...
                if item[0] == 'u': #user
                    ...
                    ao_member_user = get_allowed_objects(user, Community, ['member', 'manage'])
                ...
                for k in dest:
                    if isinstance(k, User):
                        ...
                        if k.email_notification:
                            users = list(chain(get_acl_by_object(k, 'member'), get_acl_by_object(k, 'manage')))
            ...
            if len(upload_to) and len(files):  
                try:
                    ...
                    for f in request.FILES.getlist("attachments"):
                        ...
                        fname = slugify(s[0])
                        fext = s[1] 
                        destination = open(u"%s/%s" % (path, u"%s%s" % (fname, fext)), 'wb+')
                        ...
                        for d in upload_to:
                            ...
                            try:
                                libfile = ZornaFile(owner=request.user, modifier=request.user)
                                libfile.save() 
                                fsrc = u"%s/%s/%s,%s%s" % (path_library,d, str(libfile.pk), fname, fext)
                                shutil.copy2(u"%s/%s" % (path, u"%s%s" % (fname, fext)), fsrc)
            ...
            if len(users_emails):
                ...
                if users_emails:       
                    email = ZornaEmail()
                    url = request.build_absolute_uri(reverse('communities_home_page', args=[])) + '?all_msg=message&message_id=%s' % m.pk
                    ec = {"message": m, 'url': url, 'user': request.user}
                    body_text = render_to_string('communities/email_notification_text.html', ec )
                    body_html = render_to_string('communities/email_notification_html.html', ec )
                    subject = _(u'A new message has been posted in communities')
                    step = getattr(settings, "ZORNA_MAIL_MAXPERPACKET", 25)
                    for n in range(0, len(users_emails)/step + 1):
                        email.append(subject, body_text, body_html, settings.DEFAULT_FROM_EMAIL, bcc=users_emails[n*step:(n+1)*step])
                    email.send()
...
class InviteCommunityForm(forms.Form):
    send_to = forms.CharField(widget=forms.TextInput(attrs={'class':'com_users'}), label=_(u'Choose'), help_text=_(u"Enter names"))
    manager = forms.BooleanField(label=_(u"Manager"), initial=False, required=False, help_text=_(u"Check this if you want to invite these people as managers"))
    message = forms.CharField(widget=forms.Textarea(attrs={'rows':'5', 'cols':'40'}), help_text=_(u"Tell them why you're adding them"))



class EventCommunityForm(ModelForm):
    title = forms.CharField()
    start = forms.DateTimeField(label=_("Start date"),widget=forms.SplitDateTimeWidget)
    end = forms.DateTimeField(label=_("End date"),widget=forms.SplitDateTimeWidget, help_text = _("The end time must be later than start time."))
    ...
    def clean(self):
        ...
        if not 'start' in self.cleaned_data or not 'end' in self.cleaned_data:
            raise forms.ValidationError(_(u'You must specify start and end dates'))
        elif start > end :
    ...
    def save(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            event_data = {  'title': self.cleaned_data['title'], 
    ...
    def clean(self):
        if not self.instance.pk:
            ...
            if self.poll_form_set.total_form_count() < 2 or not self.poll_form_set.is_valid():
                raise forms.ValidationError(_(u'You must specify at least two choices'))
            else:
...
class UrlCommunityForm(ModelForm):
    ...
    def clean(self):
        ...
        if settings.EMBEDLY_KEY and EMBEDLY_RE.search(url):
            ...
            raise forms.ValidationError(_(u'Invalid url or script embed codes are not supported'))
        return self.cleaned_data        
    ...
    def save(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            if self.url_summary:
                uc = UrlCommunity(url=self.cleaned_data['url'], title=self.url_summary[0], excerpt=self.url_summary[1])
            else:
                uc = UrlCommunity(url=self.cleaned_data['url'], title='', excerpt='')
            uc.save()
        return uc
