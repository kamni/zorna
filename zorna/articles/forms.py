...
class ArticleCategoryForm(ModelForm):
    parent = TreeNodeChoiceField(
        queryset=ArticleCategory.tree.filter(), required=False, empty_label="-- Nothing --")
...
class ArticleAttachmentsForm(ModelForm):
    attached_file = forms.CharField(label=_(
...
class ArticleStoryForm(ModelForm):
    image = forms.ImageField(
        required=False, widget=forms.FileInput(attrs={'size': '80'}))
    title = forms.CharField(label=_(
        u'title'), widget=forms.TextInput(attrs={'size': '80'}))
    slug = forms.SlugField(label=_(u'slug'), widget=forms.TextInput(
    ...
    def clean_slug(self):
        if 'slug' in self.cleaned_data and 'title' in self.cleaned_data:
            ...
            raise forms.ValidationError(_(
                u'You must provide a slug or a story title'))
