...
def get_allowed_objects(user, model, permission):
    if type(permission) is list:
        ...
        for perm in permission:
            ao = ao.union(set(ACLPermission.objects.get_acl_objects_by_model(user, model, perm)))
        return list(ao)
    else:
...
class ACLPermissionManager(models.Manager):
    ...
    def copy_permissions(self, obj_source, perm_source, obj_target, perm_target):
        ...
        self.filter(object_id=obj_target.pk, content_type=ct_target, permission__codename=perm_target).delete()
        perms = self.filter(object_id=obj_source.pk, content_type=ct_src, permission__codename=perm_source)
    ...
    def cache_acl_groups(self, object):
        ...
        if acl_groups_permissions == None:
        ...
        perms = self.filter(group__isnull=False, object_id=object.pk, content_type=ct).values('permission__codename', 'content_type', 'object_id', 'group', 'acltype').order_by('group__lft')       
        for p in perms:
            try:
                groups = acl_groups_permissions[ct.pk][object.id][p['permission__codename']]
            except:
                groups = acl_groups_permissions[ct.pk][object.id][p['permission__codename']] = []
            ...
            if p['group'] > defines.ZORNA_GROUP_REGISTERED:
                if p['acltype'] == defines.ZORNA_PERMISSIONS_DENY_CHILDS or p['acltype'] == defines.ZORNA_PERMISSIONS_ALLOW_CHILDS: # Deny++ and Allow++
                    if not self.cache_childs_groups.has_key(p['group']):
                        self.cache_childs_groups[p['group']] = UserGroup.objects.get(pk=p['group']).get_descendants(True)
                    childs = self.cache_childs_groups[p['group']]
                    childs_id = [g.pk for g in childs ]
                    if p['acltype'] == defines.ZORNA_PERMISSIONS_DENY_CHILDS:
                        groups =  [groups - childs_id for groups,childs_id in zip(groups, childs_id)]
                    else:
            ...
            acl_groups_permissions[ct.pk][object.id][p['permission__codename']] = groups
    ...
    def get_acl_by_object(self, object, permission):
        ...
        if defines.ZORNA_GROUP_PUBLIC in allowed_groups or defines.ZORNA_GROUP_REGISTERED in allowed_groups:
            members = User.objects.filter(is_active=1).order_by('first_name', 'last_name')
        else:
            ct = ContentType.objects.get_for_model(object) 
            members = User.objects.filter(Q(user_profile__groups__in=allowed_groups)|Q(aclpermission__permission__codename=permission, aclpermission__content_type=ct, aclpermission__object_id=object.pk)).distinct().order_by('first_name', 'last_name')
        return members
    ...
    def get_acl_users_by_object(self, object, permission):
        ...
        """
        ct = ContentType.objects.get_for_model(object) 
        members = User.objects.filter(Q(aclpermission__permission__codename=permission, aclpermission__content_type=ct, aclpermission__object_id=object.pk)).distinct().order_by('first_name', 'last_name')
        return members
    ...
    def cache_user_permissions(self, user):
        ...
        #p = ACLPermission.objects.select_related().filter(group__isnull=False).values('permission__codename', 'object__content_type', 'object', 'group', 'acltype')
        
        perms = self.filter(group__isnull=False).values('permission__codename', 'content_type', 'object_id', 'group', 'acltype')       
        for p in perms:
            ...
            if not acl_users_permissions[user_id][p['content_type']].has_key(p['object_id']):
                acl_users_permissions[user_id][p['content_type']][p['object_id']] = {}
            ...
            if p['group'] == defines.ZORNA_GROUP_PUBLIC: # Public
                ...
                bok = True
            elif p['group'] == defines.ZORNA_GROUP_REGISTERED and user.is_anonymous() == False:
                bok = True
            ...
            if bok:
                acl_users_permissions[user_id][p['content_type']][p['object_id']][p['permission__codename']]= True if p['acltype'] == defines.ZORNA_PERMISSIONS_ALLOW_CHILDS or p['acltype'] == defines.ZORNA_PERMISSIONS_ALLOW else False
                acl_users_permissions[user_id][p['content_type']][p['object_id']]['ct']= p['content_type']
                

        if user_id:                       
            perms = self.filter(user=user).values('permission__codename', 'content_type', 'object_id', 'group', 'acltype')        
            for p in perms:
                if not acl_users_permissions[user_id].has_key(p['content_type']):
                    acl_users_permissions[user_id][p['content_type']] = {}
                if not acl_users_permissions[user_id][p['content_type']].has_key(p['object_id']):
                ...
                acl_users_permissions[user_id][p['content_type']][p['object_id']][p['permission__codename']] = True if  p['acltype'] == defines.ZORNA_PERMISSIONS_ALLOW else False                
                acl_users_permissions[user_id][p['content_type']][p['object_id']]['ct']= p['content_type']
    ...
    def delete_user_permissions(self, permission, obj):
        ct = ContentType.objects.get_for_model(obj)
        ACLPermission.objects.filter(permission__codename=permission, object_id=obj.pk, content_type=ct, user__isnull=False).delete()
...
class ACLPermission( models.Model ):
    ...
    acltype = models.IntegerField(max_length=1, choices=PERMISSIONS_TYPES, default=0)
    ...
    class Meta:
        ...
        unique_together = (('permission', 'object_id', 'content_type', 'user', 'group'),)
...
class ACLMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for perm in perms:
            if current_perms.has_key(perm.codename) == False:
...
class BaseACL(object):
    ...
    def get_acl_groups_forms(self, request, object_id, **kwargs):
        ...
        if request.method == 'POST':
            if not request.POST.has_key('bupdate'):
                ...
                ACLPermission.objects.filter(object_id=obj.pk, content_type=ct, group__isnull=False).delete()
                temp = {}
                for ac in request.POST:
                    if ac[:5] == 'perm_' and request.POST[ac] != "":
                        acc = ac.split('_')
                        try:
                            temp[int(acc[2])].append( (acc[1], request.POST[ac]) )
                        except KeyError:
                            temp[int(acc[2])] = []
                            temp[int(acc[2])].append( (acc[1], request.POST[ac]) )
        ...
        #perm =  perm_object.aclpermission_set.select_related().filter(group__isnull=False)
        perm =  ACLPermission.objects.select_related().filter(object_id=perm_object.pk, content_type=ct, group__isnull=False)
        parentsid = [];      
        for ob in ob_list:
        ...
        #e = ZornaEntity.objects.filter(aclpermission__object__content_type=ContentType.objects.get_for_model(obj)).distinct()
        extra_context['perm_objects'] = {}
        #for c in e:
        #    c = c.as_leaf_class() #TODO A optimiser
        #    extra_context['perm_objects'].append(c)
        
        perm =  ACLPermission.objects.select_related().filter(content_type=ct, group__isnull=False).exclude(object_id=obj.pk)
        for p in perm:
            try:
                extra_context['perm_objects'][p.content_object.pk] = p.content_object
            except:
                pass
    ...
    def get_acl_users_forms(self, request, object_id, template = 'acl/user_acl_users.html'):
        ...
        if request.method == 'POST':
            ...
            # delete permission
            #obj.aclpermission_set.filter(object=obj, user__isnull=False).delete()
            ACLPermission.objects.filter(object_id=obj.pk, content_type=ct, user__isnull=False).delete()
        ...
        ob_list = User.objects.filter(aclpermission__user__isnull=False, aclpermission__object_id=obj.pk, aclpermission__content_type=ct).distinct()
        #perm =  obj.aclpermission_set.select_related().filter(object=obj, user__isnull=False)
        perm =  ACLPermission.objects.select_related().filter(object_id=obj.pk, content_type=ct, user__isnull=False)
        dummy = ob_list
        for ob in dummy:
            ob.verbs = {}
            for k,v in self.perms.iteritems():
        ...
        paginate_by=20
        paginator = Paginator(ob_list._clone(), paginate_by, allow_empty_first_page=True)
        ...
        extra_context['acl_form'] = render_to_string(acl_template, extra_context, context_instance=RequestContext(request))
        context = RequestContext(request)
        if template:
            return render_to_response(template, extra_context, context_instance=context)
        else:
    ...
    def remove_perm(self, check, obj, user_or_group, acltype):
        ...
        if isinstance(user_or_group, User):
            ...
            return False
        content_type = ContentType.objects.get_for_model(obj)    
        ACLPermission.objects.filter(object_id=obj.pk, content_type=content_type, user=user_or_group, permission=self.perms[check]).delete()
        cache.delete(ACL_USERS_PERMISSIONS_CACHE)
        cache.delete(ACL_GROUPS_PERMISSIONS_CACHE)
        return True
