...
class user_menus( template.Node ):
    ...
    def render(self, context):
        ...
        if bcom_access1 or bcom_access2:
            ...
            ret.append(p)
        bpersonal_documents = SiteOptions.objects.is_access_valid(request.user, 'zorna_personal_documents')
        if bpersonal_documents or bcom_access1:
            p = {'url': reverse('documents'), 'text': _(u'Documents')}
            ret.append(p)
        else:
            ao = get_allowed_objects(request.user, ZornaFolder, ['writer', 'reader', 'manager'])
            if ao:
                p = {'url': reverse('documents'), 'text': _(u'Documents')}
                ret.append(p)
        ...
        if ao:
            p = {'url': reverse('writer_stories_list'), 'text': _(u'Publication')}
            ret.append(p)

        b_pages_manager = SiteOptions.objects.is_access_valid(request.user, 'zorna_pages_pages')
        b_templates_manager = SiteOptions.objects.is_access_valid(request.user, 'zorna_pages_templates')
            
        if b_pages_manager or b_templates_manager:
            p = {'url': reverse('fmbrowser_home'), 'text': _(u'Pages')}
            ret.append(p)
            
        b_menus_manager = SiteOptions.objects.is_access_valid(request.user, 'zorna_menus')
            
        if b_menus_manager:
            p = {'url': reverse('menus_home_view'), 'text': _(u'Menus')}
            ret.append(p)

        cals = get_user_calendars(request.user, ['viewer', 'manager', 'creator'])
        if cals:
        ...
        if ao:
            ...
            p = {'url': reverse('forms_home', args=[fw.slug]), 'text': _(u'Forms')}
            ret.append(p)
...
class admin_menus( template.Node ):
    ...
    def render(self, context):
        request = context['request']
        ret = []
        bvalidator = SiteOptions.objects.is_access_valid(request.user, 'zorna_validate_registration')
        if request.user.is_superuser:
            ret.append({'url': reverse('admin_list_categories'), 'text': _(u'Articles')})
            ret.append({'url': reverse('admin_list_communities'), 'text': _(u'Communities')})
            ret.append({'url': reverse('admin_list_calendars'), 'text': _(u'Calendars')})
            ret.append({'url': reverse('list_faqs'), 'text': _(u'Faqs')})
            ret.append({'url': reverse('admin_list_folders'), 'text': _(u'Folders')})
            ret.append({'url': reverse('list_workspaces'), 'text': _(u'Forms')})
            ret.append({'url': reverse('admin_list_alerts'), 'text': _(u'Site')})
