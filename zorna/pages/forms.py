...
class PageEditFileForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        for b in extra:
            self.fields['%s' % b['block']] = forms.CharField(widget=CKEditorWidget(), initial=b['content'])

class PageEditFileContextForm(forms.Form):
    title = forms.CharField(_(u'title'), widget=forms.TextInput(attrs={'size':'80'}))
    keywords = forms.CharField(_(u'keywords'), widget=forms.TextInput(attrs={'size':'80'}))
    description = forms.CharField(_(u'description'), widget=forms.Textarea(attrs={'rows':'5', 'cols':'80'}), required=False)
