...
def forms_view_design_form(request, form):
    ...
    c = RequestContext(request, {"form": form, "form_for_form": form_for_form, "form_design": True})
    return t.render(c)
...
def get_forms_navigation(request, slug, form=None):
    ...
    if fw:
        ...
        if len(ob_list) and form == None:
            current_form = ob_list[0]
        else:
            current_form = form
        lists = FormsList.objects.filter(workspace=fw)
        c = RequestContext(request, {"forms_list":ob_list, 'workspace': fw,'current_form':current_form, 'lists':lists})
        return t.render(c)
    else:
        return ''
    ...
    if request.user.is_authenticated():
        ...
        if fw:
            ...
            if form:
                ...
                try:
                    extra_context['url_frame'] = reverse('forms_form', args=[FormsForm.objects.filter(workspace=fw)[0].pk])
                except:
...
def forms_form_print(request, form, entry=None, extra={}):
    ...
    extra_context['controls_multiple'] = ['CheckboxSelectMultiple', 'RadioSelect', 'SelectMultiple']
...
def add_bind_entry_field(form, bind):
    ...
    if r[1] == 'zorna_owner':
        label = _(u"User")
        form_field = FormsFormField(form=form, label=label, slug=slug, field_type=fields.ZORNA_USER_SINGLETON)
        form_field.save()
    else:
        field = FormsFormField.objects.get(slug=r[1], form__slug=r[0])
        label = field.label
        form_field = FormsFormField(form=form, label=label, slug=r[1], field_type=fields.FORM_ENTRY)
        form_field.save()

def delete_bind_entry_field(form, bind):
    r = bind.split('.')
    if r[1] == 'zorna_owner':
        FormsFormField.objects.get(form=form, field_type=fields.ZORNA_USER_SINGLETON).delete()
    else:
        FormsFormField.objects.get(form=form, field_type=fields.FORM_ENTRY).delete()
...
def update_bind_entry_field(form, bind):
    ...
    if r[1] != 'zorna_owner':
        field = FormsFormField.objects.get(slug=r[1], form__slug=r[0])
        label = field.label
        form_field = FormsFormField.objects.get(form=form, field_type=fields.FORM_ENTRY)
        ...
        if request.method == 'POST' :
            ...
            if form_form.is_valid():
                ...
                if not bind_to_account and f.bind_to_account:
                    ...
                    FormsFormField.objects.get(form=f, field_type=fields.ZORNA_USER_SINGLETON ).delete()
    ...
    if request.user.is_authenticated():
        ...
        if request.method == 'POST' :
            ...
            if field_form.is_valid():
                fmax = FormsFormField.objects.filter(form=form).aggregate(max_sort_order= Max('sort_order'))
                max_order= fmax['max_sort_order'] if fmax['max_sort_order'] else 0 
                fmax = FormsFormField.objects.filter(form=form).aggregate(max_sort_order= Max('sort_order_list'))
                max_order_list= fmax['max_sort_order'] if fmax['max_sort_order'] else 0 
                ...
                if request.POST.has_key('bcontinue'):
                    #return HttpResponseRedirect(reverse('forms_form_add_field', args=[form.pk]))
        ...
        field_form = FormsFormFieldForm(request.POST or None, initial={'form':form})
        field_form.fields['list'].queryset = FormsList.objects.filter(workspace = form.workspace)
        field_form.fields['panel'].queryset = FormsFormPanel.objects.filter(form = form)
...
def forms_form_edit_field(request, field):
    if request.user.is_authenticated():
        try:
            field = FormsFormField.objects.select_related(depth=1).get(pk=field)
        ...
        if field.field_type >= fields.ZORNA_USER_SINGLETON:
            ...
            field_form.fields['list'].queryset = FormsList.objects.filter(workspace = field.form.workspace)
        field_form.fields['panel'].queryset = FormsFormPanel.objects.filter(form = field.form)
...
def forms_lists_list(request, slug, extra = {}):
    if request.user.is_authenticated():
        ...
        if fw:        
            extra_context = dict(**extra)
            t = loader.get_template('forms/forms_path.html')
            c = RequestContext(request, {'path_url': reverse('forms_lists_list', args=[slug]), 'path_url_text': _(u'Lists'), 'item_name':None})
            extra_context['forms_path'] = t.render(c)
                ...
                if form_set.is_valid():
                    ...
                    for i in range(0, form_set.total_form_count()):
                        ...
                        try:
                            ...
                            if value:
                                entry = FormsListEntry(value = form.cleaned_data['value'])
                                entry.list = fl
                                entry.save()
                        except:
                            pass
...
def forms_edit_list(request, list):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST' :
            ...
            entry_form_set = inlineformset_factory(FormsList, FormsListEntry, can_delete=True)
            ...
            entry_form_set = inlineformset_factory(FormsList, FormsListEntry, can_delete=True, extra = 1)
        ...
        t = loader.get_template('forms/forms_path.html')
        c = RequestContext(request, {'path_url': reverse('forms_lists_list', args=[fw.slug]), 'path_url_text': _(u'Lists'), 'item_name':list.name})
        extra_context['forms_path'] = t.render(c)
...
def forms_edit_form_entry(request, entry):
    ...
    if request.method == "POST":
        if request.POST.has_key('bdelete') and bdelete:
    ...
    extra_context = {"form": form, "form_for_form": form_for_form, 'bdelete': bdelete}
    ...
    if entry.entry:
        try:
            ...
            while entry.entry:
                e = FormsFormEntry.objects.select_related().get(pk= entry.entry.pk, form__slug=bte[0])
...
def forms_add_form_entry(request, slug):
    ...
    if rget['where']:
        try:
            ...
            pk = d[1]
            while True:
                e = FormsFormEntry.objects.select_related().get(pk= pk, form__slug=bte[0])
                c, r = forms_get_entry(e)
                extra_context['parents'].append({'row': r[bte[1]], 'entry': e})
                if not e.form.bind_to_entry:
                    break;
    ...
    if request.user.is_authenticated():
        ...
        if tempfile:
            ...
            for row in csv_reader:
                ...
                if rows == 0:
                    for col in row:
                        ...
                        ft = request.POST.get('field_type_%s' % i, str(fields.TEXT))
                        ...
                        if ft == '0': #ignore
                            ...
                            fl = FormsList.objects.create(name=col, workspace=fw)
                            lists[fl.pk] = []
                            col_lists[i] = fl.pk
                            fl = FormsFormField.objects.create(form=form, label=col, slug=col_slug, required=1, field_type=fields.SELECT, list=fl)
                        elif ft[0:2] == 'f-':
                            try:
                                fl = form.fields.get(pk=int(ft[2:]))
                                if fl.reference:
                                    r = fl.reference.split('.')
                                    fl.form_target = FormsForm.objects.get(slug=r[0])
                                    fl.slug_target = r[1]
                                    fl.target_entries = {}
                                elif fl.field_type in fields.LISTS:
                                    flist = FormsList.objects.create(name=col, workspace=fw)
                                    lists[flist.pk] = []
                                    col_lists[i] = flist.pk
                                    fl.list = flist
                                    fl.save()
                            except Exception as e:
                                fl = FormsFormField.objects.create(form=form, label=col, slug=col_slug, required=1, field_type=fields.TEXT)
                        elif ft[0:3] == 'ff-': #bind_entry
                            try:
                                if form.bind_to_entry:
                                    fl = FormsFormField.objects.get(pk=int(ft[3:]))
                            ...
                            fl = FormsFormField.objects.create(form=form, label=col, slug=col_slug, required=1, field_type=int(ft))
                        if fl:
                    ...
                    for idx, fl in fields_columns.iteritems():
                        try:
                            if fl.form_target:
                                if not col_targets.has_key(fl.form_target.pk):
                                    col_targets[fl.form_target.pk] = FormsFieldEntry.objects.forms_get_entries(fl.form_target)
                                entries = col_targets[fl.form_target.pk][1]
                                for e in entries:
                                    fl.target_entries[e[fl.slug_target]['value'].lower()] = e['entity'].pk
                        except Exception as e:
                    ...
                    for col in row:
                        #create field entry
                        col = col.strip()
                        if fields_columns.has_key(i):
                            col = smart_unicode(col, encoding)
                            lower_col = col.lower()
                            if fields_columns[i].target_entries and fields_columns[i].target_entries.has_key(lower_col):
                                col = fields_columns[i].target_entries[lower_col]
                                try:
            ...
            for i,row in form_entries:
                kwargs = {'form': form, 'modifier':request.user, 'owner':request.user}
                if not col_bind_account == None and users.has_key(row[col_bind_account]):
                    kwargs['account_id'] = users[row[col_bind_account]]
                if form_bind_entries.has_key(i):                   
            ...
            for k,row in form_entries:                     
                i = 0
                #entry = FormsFormEntry.objects.create(form=form, modifier=request.user)
                for col in row:
                    col = col.strip()
                    #create field entry
                    if fields_columns.has_key(i):
                        try:
                            ...
                            if fields_columns[i].target_entries:
                                lower_col = col.lower()
                                if fields_columns[i].target_entries.has_key(lower_col):
                                    col = fields_columns[i].target_entries[lower_col]
                                else:
                                    col = None
                            elif not col and fields_columns[i].default_value:
                                col = fields_columns[i].default_value  
                            #entry.fields.create(field=fields_columns[i], value=col)
                            if not col == None:
                                bulk_values.extend([fields_columns[i].pk, rows_entries[k].pk, col])
            ...
            arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fe_fields)) + ')'] * (len(bulk_values)/len(fe_fields)))                   
            sql = "INSERT INTO %s (%s) VALUES %s" % (FormsFieldEntry._meta.db_table, flds, arg_string,) 
...
def form_map_import_fields(request, form, tempfile, separator, encoding='utf-8' ):
    if request.user.is_authenticated():
        ...
        import csv
        extra_context = {}
        csv_reader = csv.reader(open(tempfile, 'rb'), delimiter=smart_str(separator))
        for headers in csv_reader:
            break;
        i = 0
        extra_context['csv_header'] = []
        for col in headers:
            extra_context['csv_header'].append({ 'id':i, 'label':smart_unicode(col, encoding)})
            i = i + 1
        ...
        extra_context['type_choices'] = [
                                        {'id': fields.TEXT, 'label': _("Single line text")},
                                        {'id': fields.TEXTAREA, 'label': _("Multi line text")},
                                        {'id': fields.EMAIL, 'label': _("Email")},
                                        {'id': fields.DATE, 'label': _("Date")},
                                        {'id': fields.DATE_TIME, 'label': _("Date/time")},
                                        {'id': fields.SELECT, 'label': _("List")},
                                        ]
        ...
        for f in form_fields:
            if f.field_type == fields.ZORNA_USER_SINGLETON:
                extra_context['field_choices'].append({'id': 'id_zorna_account', 'label': _(u'Nickname')})
            elif f.field_type == fields.FORM_ENTRY:
                bte = form.bind_to_entry.split('.')
                ff = FormsFormField.objects.get(slug=bte[1], form__slug=bte[0])
                extra_context['field_choices'].append({'id': 'ff-%s' % ff.pk, 'label': ff.label})
            else:
                extra_context['field_choices'].append({'id': 'f-%s' % f.pk, 'label': f.label})
...
def form_import_fields(request, form):
    if request.user.is_authenticated():
        try:
            ...
            return HttpResponseForbidden()
        form_csv = FormsFormImportCsv(request.POST or None, request.FILES or None)
        if request.method == 'POST':
...
def forms_table_order_fields(request, form):
    if request.user.is_authenticated():
        ...
        fields = [int(f) for f in request.GET.getlist('field-table-dnd[]') if f]
...
def forms_edit_form_template(request, form):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST' :
            ...
            tpl_form = FormsFormTemplateForm(initial={'template': form.template})
...
def forms_form_export_entries(request, form):
    if request.user.is_authenticated():
        ...
        extra_context = {"title": _("Export Entries"), "export_form": export_form,
                   "submitted": submitted}
        extra_context['zorna_title_page'] = _(u'Forms')
        extra_context['current_form'] = form
...
def file_view(request, file):
    ...
    """
    try:
        field_entry = FormsFieldEntry.objects.select_related(depth=1).get(pk=file)
...
def forms_form_view_entry(request, entry):
    ...
    if row['entity'].account_id:
        extra_context['user_avatar'] = row['entity'].get_user_avatar(row['entity'].account_id)
    else:
...
def browse_references(request, workspace_slug):
    ...
    if fw:
        ...
        extra_context = {'forms': forms, 'target': request.GET.get('target', '')}
        context = RequestContext(request)
        ...
        if request.method == 'POST' :
            ...
            if tpl_form.is_valid():
                ...
                try:
                    ...
                    FormsFormAction.objects.create(form=form, content_object=ffam)
...
def forms_form_panel_add(request, form_pk):
    if request.user.is_authenticated():
        ...
        if request.method == 'POST' :
            ...
            if ff.is_valid():
                fmax = FormsFormPanel.objects.filter(form=form).aggregate(max_sort_order= Max('sort_order'))
                max_order= fmax['max_sort_order'] if fmax['max_sort_order'] else 0 
...
def forms_form_panel_edit(request, panel_pk):
    if request.user.is_authenticated():
        try:
            panel = FormsFormPanel.objects.select_related(depth=1).get(pk=panel_pk)
            form = panel.form
            fw = isUserManager(request, form.workspace.slug)
            if fw == False:
                return HttpResponseForbidden()
