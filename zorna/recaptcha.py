...
Derive every form that has the captcha field from ReCaptchaForm and when you create the form object after receiving POST/GET, pass a remoteip
parameter like that:
...
class ReCaptchaWidget(forms.Widget):
    ...
    def get_recaptcha_widget_html(self, field_name):
        ...
        widget += u'Recaptcha.create("%s", "fdiv_%s", {theme: "white", lang: "%s"});' % (
            settings.RECAPTCHA_PUBKEY, field_name, settings.LANGUAGE_CODE[0:2])
        widget += u'</script>'
...
class ReCaptchaField(forms.Field):
    ...
    def clean(self, value):
        ...
        result = self.verify_captcha(value[
                                     'challenge'], value['response'], value['remoteip'])
